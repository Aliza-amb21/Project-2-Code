...
All filtered fragment proportion data in one data frame
```{r}
...
All insert size data in one data frame
```{r}
...
#filter the clinical data to get only samples where 'sample type' = cfDNA
filtered_samples<-filter(samples,sample_type=="cfDNA")

#to get all sWGS samples
WGS_samples<-filter(filtered_samples,sWGS!="No") # don't run for SNP data

#to get all samples that have sWGS, TMB, and SNP
WGS_samples<-filter(filtered_samples,sWGS!="No" & Pan_cancer_panel!="No" & SNP_panel!="No") # don't run for SNP data

#to get all samples that have sWGS and SNP
WGS_samples<-filter(filtered_samples,sWGS!="No" & SNP_panel!="No") # don't run for SNP data
...
#just cfDNA + non-gestational samples
...
#for samples with sWGS and SNP data
filtered_all_sWGS_TMB_SNP<-filtered_combined_all_s2

#run for SNP data
filtered_sWGS_all_s2_SNP<-filtered_combined_all_s2

#just cfDNA samples (sWGS, TMB, SNP)
...
#just non-gestational cfDNA samples (sWGS and TMB)
...
Set plot theme for insert size proportion plots
```{r}
theme_set(
  theme_bw()
  )
...
```{r}
#order the estimated tumour content
...
ggsave("sWGS_FacetWrap_Patient_InsertSize_vs_Proportion_ColourBy_EsTumContent_v1_3.png", width = 15, height = 10, dpi=700)

ggsave("sWGS_FacetWrap_Patient_InsertSize_vs_Proportion_ColourBy_EsTumContent_v2_3.png", width = 15, height = 10, device = "tiff", dpi=700)
...
ggsave("SNP_FacetWrap_Patient_InsertSize_vs_Proportion_ColourBy_EsTumContent_v1_1.png", width = 20, height = 10, dpi=700)

ggsave("SNP_FacetWrap_Patient_InsertSize_vs_Proportion_ColourBy_EsTumContent_v2_1.png", width = 20, height = 10, device = "tiff", dpi=700)
...
Fragment size profile plots for samples with data from sWGS and SNP)
```{r}
#order the estimated tumour content
...
#plot 6 - filtered (<1000 bp) and unfiltered insert size plots
...
#plot 10 All
ggplot(filtered_all_sWGS_TMB_SNP, aes(x=insert_size, y=proportion, color=Patient))+
  geom_point(size = 10)+
  labs(fill = "Tumour Origin")+
        theme(text = element_text(size = 40),
          axis.text.x = element_text(size=35),
          axis.text.y = element_text(size=35),
          #panel.grid.minor.x = element_blank(),
...
ggsave("All_FacetGrid_Tumour_Exp_InsertSize_vs_Proportion_ColourBy_Est.Tum.Con_v1_2.png", width = 30, height = 10, dpi=700)

ggsave("All_FacetGrid_Tumour_Exp_InsertSize_vs_Proportion_ColourBy_Est.Tum.Con_v2_2.png", width = 30, height = 10, device = "tiff", dpi=700)
...
Global fragment size analysis

For loop boxplot
- Boxplots for all fragment ranges - sWGS, TMB, and SNP
```{r}
#assign the type of data
...
#different fragment ranges optimising the <150 bp fragment range
...
Boxplots extended (with p-values)
```{r}
...
b1<-ggplot(Fragment_data2, aes(x=Tumour_origin, y=Proportion_below_150bp), size = 30) +
        ...
        geom_point(aes(fill = Estimated_tumour_percent_bin), size = 6, shape = 21)+
        labs(fill = "Estimated ctDNA (%)")+
        theme(text = element_text(size = 35),
          axis.text.x = element_text(size=30, angle=65, hjust=1),
        ...
        ylab("P(<150bp)")+
        #ggtitle("sWGS")+
        stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", label = "p.signif", size = 15)+
        #stat_compare_means(comparisons = my_comparisons, method = "wilcox.test")+
...
#save boxplot
ggsave("SNP_boxplot_4_v1_1.png", width = 40, height = 18, device = "tiff", dpi=700)

ggsave("SNP_boxplot_4_v2_2.png", width = 40, height = 18, dpi=700)

```

For loop boxplot with Kruskal-Wallis test
```{r}
Fragment_data <- filtered_sWGS_all_s2_cfDNA
Fragment_data <- filtered_sWGS_all_s2_TMB
Fragment_data <- filtered_sWGS_all_s2_SNP
...
#combine plots into one (with statistical test)
...
#combine plots into one (without statistical test)
...
Correlation plots (fragment ranges) 
- Estimated tumour content
```{r}
Fragment_data <- filtered_sWGS_all_s2_cfDNA
Fragment_data <- filtered_sWGS_all_s2_TMB
...
#exclude normal control samples from data
...
#combine all plots into one (with statistical test)
ETC_correlation_figure <- ggarrange(Sp1, Sp2, Sp3, Sp4, ncol = 4, nrow = 1)

#combine all plots into one (without statistical test)
...
#exclude normal and non-gestational samples
...
#show statistical test based on tumour origin
...
#save plot
ggsave("SNP_hcG_correlation_TumourOrigin_updated_v1_1.png", width = 49, height = 15, dpi=700)

ggsave("SNP_hcG_correlation_TumourOrigin_updated_v2_1.png", width = 49, height = 15, device = "tiff", dpi=700)


```

sWGS correlation - serum hCG 
- all points on one plot
```{r}
new_sc1 <- ggplot()+
...
#save plot
ggsave("sWGS_hcG_correlation_TumourOrigin_v1_1.png", width = 49, height = 15, dpi=700)

ggsave("sWGS_hcG_correlation_TumourOrigin_v2_2.png", width = 49, height = 15, device = "tiff", dpi=700)
...
#another sequence of combination of proportions to loop through
...
#SNP comparisons for statistical analysis
my_comparisons<- list(c("Normal hCG", "Gestational (molar)"), c("Normal hCG", "Gestational (non-molar)"), c("Normal hCG", "Non-gestational"), c("Gestational (non-molar)", "Gestational (molar)"), c("Non-gestational", "Gestational (molar)"), c("Non-gestational", "Gestational (non-molar)"))
...
for(i in Fragment_name_indices) {     # Printing ggplot within for-loop
  print(ggplot(Fragment_combinations_boxplot, aes(x = Tumour_origin, y = Fragment_combinations_boxplot[, i])) +
        ...
        theme(text = element_text(size = 40),
          panel.grid.minor.x = element_blank(),
          panel.grid.major.x = element_blank(),
          axis.text.x = element_text(size=35, angle=65, hjust=1),
          axis.text.y = element_text(size=35),
          legend.position = "right",
          legend.text = element_text(size = 30),
          legend.title = element_text(size = 30))+
        ...
        stat_compare_means(label.y = 0)+
        ggtitle("SNP"))
  ggsave(filename=str_c(colnames(Fragment_combinations_boxplot)[i], "_fragment_combination_ratio_boxplot_SNP_v1.png"), width = 15, height = 18, dpi=700)
  #Sys.sleep(2)
}
...
#excluding normal and non-gestational samples
...
#save plot
ggsave("SNP_hcG_correlation_TumourOrigin_ratio_v1_1.png", width = 20, height = 15, dpi=700)

ggsave("SNP_hcG_correlation_TumourOrigin_ratio_v2_1.png", width = 20, height = 15, device = "tiff", dpi=700)
...
#group statistical test by tumour origin
...
Stacked bar chart, boxplot, and correlation plot 
...
IchorCNA data - tumour content plots (only for sWGS data)
```{r}
...
#exclude samples with ichorCNA tumour fraction as 0% prior to filtering
ichorCNA_data_sWGS<-filter(ichorCNA_data_sWGS, ichorCNA_tumour_fraction_all != 0)

#using the same data as above, exclude normal control samples
...
ggplot(ichorCNA_data_sWGS, aes(x=Estimated_tumour_fraction, y=ichorCNA_tumour_fraction_all, color=Tumour_origin))+
  ...
  ylim(0, 0.7)+
  xlim(0, 0.9)+
  xlab("Original Estimated Tumour Fraction")+
  ylab("ichorCNA Tumour Fraction")
  
ggsave("sWGS_Tumour_fraction_ichorCNA_allvsEst_v1.png", width = 20, height = 10, dpi = 700)

ggsave("sWGS_Tumour_fraction_ichorCNA_allvsEst_v2.png", width = 20, height = 10, device = "tiff", dpi = 700)
...
ggsave("sWGS_Tumour_fraction_ichorCNA_150vsAll_v1_1.png", width = 20, height = 10, device = "tiff", dpi = 700)

ggsave("sWGS_Tumour_fraction_ichorCNA_150vsAll_v2_2.png", width = 20, height = 10, dpi = 700)
...
ggplot(SNP_data_raw2, aes(x=original_fraction,  y=filtered_fraction))+ 
  geom_point(aes(fill = Tumour_origin), size = 6, shape = 21)+
  ggtitle("SNP Panel")+
  #scale_x_continuous(expand = c(0, 0), limits = c(0, NA))+
  #scale_y_continuous(expand = c(0, 0), limits = c(0, NA))+
  geom_abline(slope = 1, intercept = 0)+
...
ggsave("SNP_data_analysis_correlationplot_TumourOrigin_LineAtOrigin_v1.png", width = 15, height = 10, dpi=700)

ggsave("SNP_data_analysis_correlationplot_TumourOrigin_LineAtOrigin_v2.png", width = 15, height = 10, device = "tiff", dpi=700)
...
ggsave("SNP_data_analysis_correlationplot2_LineAtOrigin_v1.png", width = 15, height = 10, dpi=700)

ggsave("SNP_data_analysis_correlationplot2_LineAtOrigin_v2.png", width = 15, height = 10, device = "tiff", dpi=700)
...
#grouping based on tumour origin
...
#grouping based on tumour subtype
...
#heatmap coloured by tumour subtype
...
#another way of generating heatmap
